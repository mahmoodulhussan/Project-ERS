...
document.getElementById("detailed").addEventListener('click', () => {
	location.href = "../html/employee-actions.html";
});
...
let populateTable = (objList) => {
	...
	table.innerHTML = '<tr><th>REIMBURSEMENT STATUS</th><th>REIMBURSEMENT TYPE</th><th>REIMBURSEMENT AMOUNT</th><th>DESCRIPTION</th><th>DATE OF SUBMISSION</th><th>DATE OF RESOLUTION</th><th>RESOLVED BY</th></tr>';
...
//		console.log(d);
		...
		let req = await fetch('http://localhost:8080/ERS-Project/api/newreimbursement', {
			method: 'POST',
			headers: {
...
let init = async () => {
	...
	document.getElementById("re-name").innerText = `${userName}'s REIMBURSEMENT HISTORY`;
	let reimbursements = await retreiveAllReimbursements();
	populateTable(reimbursements);
}
