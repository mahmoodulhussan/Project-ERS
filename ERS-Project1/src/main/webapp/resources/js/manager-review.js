...
let populateTable = (list) => {
	...
	table.innerHTML = '<tr><th>REIMBURSEMENT STATUS</th><th>REIMBURSEMENT TYPE</th><th>REIMBURSEMENT AMOUNT</th><th>REQUESTED BY</th><th>DATE OF SUBMISSION</th><th>APPROVE/DENY</th></tr>';
...
let approve = async (id) => {
	...
	let rese = await fetch(`http://localhost:8080/ERS-Project/api/getUser?userid=${userId}`);
	...
	let res = await fetch(`http://localhost:8080/ERS-Project/api/approveReimbursement`,
	{
		method: 'POST',
		headers: {
...
let deny = async (id) => {
	...
	let res = await fetch(`http://localhost:8080/ERS-Project/api/denyReimbursement`,
	{
		method: 'POST',
		headers: {
      		'Content-Type': 'application/json'
